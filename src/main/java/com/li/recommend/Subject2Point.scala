...
object Subject2Point {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val point2question = sparkSession.sql("select _id,points,pointsName,subject,year,area from ztk_question")
      .rdd
      .filter {
        f =>
          !f.isNullAt(0) && !f.isNullAt(1) && f.getSeq[Double](1).nonEmpty && f.getSeq[Double](1).size > 2 && !f.isNullAt(4) && !f.isNullAt(5)
      }
      .mapPartitions {
        ite =>
